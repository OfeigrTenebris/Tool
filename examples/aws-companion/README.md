...
This example uses @Tool/companion with a custom AWS S3 configuration.
...
`@Tool/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @Tool-example/aws-companion start
```
