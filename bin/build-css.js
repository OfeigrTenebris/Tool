...
async function compileCSS () {
  ...
  const files = await glob('packages/{,@Tool/}*/src/style.scss')
  ...
  for (const file of files) {
    ...
    if (outputDir.includes(path.normalize('packages/Tool/'))) {
      outfile = path.join(outputDir, 'Tool.css')
    }
    await mkdir(outputDir, { recursive: true })
    await writeFile(outfile, postcssResult.css)
    console.info(
      chalk.green('✓ Built Tool CSS:'),
