...
const packageNames = await fs.readdir(new URL('../packages/@Tool', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@Tool/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@Tool/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('Tool', 2)[1]
