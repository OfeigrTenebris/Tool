describe('@Tool/react', () => {
  beforeEach(() => {
    cy.visit('/react')
    cy.get('#dashboard .Tool-Dashboard-input:first').as('dashboard-input')
    cy.get('#modal .Tool-Dashboard-input:first').as('modal-input')
    cy.get('#drag-drop .Tool-DragDrop-input').as('dragdrop-input')
  })
  ...
  it('should render Dashboard in React and show thumbnails', () => {
    ...
    cy.get('#dashboard .Tool-Dashboard-Item-previewImg')
  ...
  it('should render Dashboard with Remote Sources plugin pack', () => {
    ...
    cy.get('#dashboard .Tool-DashboardTab-name').each((item, index, list) => {
  ...
  it('should render Modal in React and show thumbnails', () => {
    ...
    cy.get('#modal .Tool-Dashboard-Item-previewImg')
      .should('have.length', 2)
      .each((element) => expect(element).attr('src').to.include('blob:'))
  })

  it('should render Drag & Drop in React and create a thumbail with @Tool/thumbnail-generator', () => {
    const spy = cy.spy()

    // eslint-disable-next-line
    // @ts-ignore fix me
    cy.window().then(({ Tool }) => Tool.on('thumbnail:generated', spy))
