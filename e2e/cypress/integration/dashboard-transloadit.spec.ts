describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ Tool }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(Tool.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(Tool.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ Tool }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      Tool.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ Tool }) => {
      ...
      const { files } = Tool.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      Tool.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ Tool }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(Tool.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        Toolserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'Tool-core:3.2.0,Tool-transloadit:3.1.3,Tool-tus:3.1.0,Tool-dropbox:3.1.1,Tool-box:2.1.1,Tool-facebook:3.1.1,Tool-google-drive:3.1.1,Tool-instagram:3.1.1,Tool-onedrive:3.1.1,Tool-zoom:2.1.1,Tool-url:3.3.1',
    ...
    cy.get('.Tool-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.Tool-StatusBar-actions > .Tool-c-btn').click()
    cy.wait([
